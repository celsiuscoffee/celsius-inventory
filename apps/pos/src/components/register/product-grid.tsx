        ...
        {products.map((product) => (
          ...
          >
            ...
            {cartCounts[product.id] > 0 && product.is_available && (
              <div className="absolute right-1 top-1 z-10 flex h-5 min-w-5 items-center justify-center rounded-full bg-brand px-1 text-[10px] font-bold text-white shadow">
                {cartCounts[product.id]}
              </div>
            )}
            {!product.is_available && (
              <div className="absolute inset-0 z-10 flex items-center justify-center rounded-lg bg-black/60">
                <span className="rounded-full bg-danger px-3 py-1 text-[11px] font-bold text-white">SOLD OUT</span>
              </div>
            )}
            ...
            <div className="px-1.5 py-1">
              <p className="text-[10px] font-medium leading-tight line-clamp-1">{product.name}</p>
              <p className="text-[10px] font-bold text-brand">{displayRM(product.price)}</p>
            </div>
          </button>
        ))}
      </div>
        ...
        <>
          ...
          >
            <div className="border-b border-border px-3 py-1.5">
              <p className="text-[11px] font-semibold">{contextMenu.product.name}</p>
              <p className="text-[10px] text-text-muted">{displayRM(contextMenu.product.price)}</p>
            </div>
            <button
              onClick={() => { if (onToggleAvailability) onToggleAvailability(contextMenu.product); setContextMenu(null); }}
              className="flex w-full items-center gap-2 px-3 py-2 text-[11px] font-medium hover:bg-surface-hover"
            >
