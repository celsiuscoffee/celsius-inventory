...
export default function SopDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: sop, mutate, isLoading } = useFetch<SopDetail>(`/api/sops/${id}`);
  const { data: categories } = useFetch<Category[]>("/api/sop-categories");
  const { data: allOutlets } = useFetch<Outlet[]>("/api/outlets");
  ...
  const saveSteps = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sops/${id}/steps`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const saveOutlets = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/sops/${id}/outlets`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  if (!sop) {
    return (
      <div className="p-6 text-center">
        ...
        <Link href="/sops" className="mt-2 text-sm text-terracotta hover:underline">Back to SOPs</Link>
      </div>
    );
  }
