...
export default function SchedulesPage() {
  const { data: schedules, isLoading, mutate } = useFetch<Schedule[]>("/api/schedules");
  const { data: sops } = useFetch<Sop[]>("/api/sops?status=PUBLISHED");
  const { data: outlets } = useFetch<Outlet[]>("/api/outlets");
  const { data: staff } = useFetch<Staff[]>("/api/staff");
