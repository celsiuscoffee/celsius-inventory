...
export async function GET() {
  try {
    ...
    console.error("[receivings GET]", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { orderId, outletId, supplierId, items, notes, status, invoicePhotos, paymentType, claimedById } = body;
    ...
    // Update stock balances + order status with error handling
    try {
      await Promise.all(
      ...
      console.error("[receivings POST] Stock adjustment or order status update failed:", stockErr);
    }
    ...
    try {
      ...
      console.error("[receivings POST] Invoice creation failed:", invoiceErr);
    }
